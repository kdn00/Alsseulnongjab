...
<body>
	...
	<c:forEach items="${diseaseinfo}" var="list">
		<c:set var="disease_name_split"
			value="${fn:split(list.disease_name, ' ')}" />
		<c:set var="disease_name_split2"
			value="${fn:split(disease_name_split[1], '(')}" />
		<c:set var="disease_name_split3"
			value="${fn:split(disease_name_split2[0], ')')}" />
